...
function options(values,current,label){return `<option value="">${esc(label)}</option>${[...new Set(values.filter(Boolean))].sort().map(v=>`<option value="${esc(v)}" ${String(v)===String(current)?'selected':''}>${esc(v)}</option>`).join('')}`}
...
export function renderSalesHistory({transactions=[],periodLabel='Periode terpilih',sortBy='qty',filters={}}={}){
  const visible=(transactions||[]).filter(tx=>matchesFilters(tx,filters)).sort((a,b)=>num(b?.ts??b?.timestamp)-num(a?.ts??a?.timestamp));
  const leaders=aggregateProductLeaderboard(visible,{sortBy}).slice(0,5),max=Math.max(1,...leaders.map(x=>sortBy==='revenue'?x.revenue:x.qty));
  const shifts=(transactions||[]).map(txShift).filter(v=>v&&v!=='-'),methods=(transactions||[]).map(txMethod).filter(v=>v&&v!=='-'),cashiers=(transactions||[]).map(txCashier).filter(v=>v&&v!=='-');
  return `<main class="sj-v26-sales-history" data-sj-v26-view="history"><header class="sj-v26-history-head"><button type="button" data-sales-action="back">${renderIcon('chevron-left',{size:18})}<span>Kembali</span></button><div><h1>Riwayat Penjualan</h1><p>${esc(periodLabel)} · ${visible.length} transaksi</p></div></header><section class="sj-v26-history-periods"><button type="button" data-sales-period="today">Hari Ini</button><button type="button" data-sales-period="yesterday">Kemarin</button><button type="button" data-sales-period="7d">7 Hari</button><button type="button" data-sales-period="month">Bulan Ini</button><label>Pilih Tanggal<input type="date" data-sales-period-date></label></section><section class="sj-v26-history-filters"><select data-sales-filter="shift">${options(shifts,filters.shift,'Semua Shift')}</select><select data-sales-filter="method">${options(methods,filters.method,'Semua Pembayaran')}</select><select data-sales-filter="cashier">${options(cashiers,filters.cashier,'Semua Kasir')}</select><input data-sales-filter="query" value="${esc(filters.query||'')}" placeholder="Cari ID transaksi / produk"></section><section class="sj-v26-leader"><div class="sj-v26-section-head"><div><h2>Produk Terlaris</h2><p>Urutkan berdasarkan jumlah atau omzet</p></div><div class="sj-v26-rank-toggle"><button type="button" data-sales-rank="qty" class="${sortBy==='qty'?'active':''}">Jumlah Terjual</button><button type="button" data-sales-rank="revenue" class="${sortBy==='revenue'?'active':''}">Omzet</button></div></div>${leaders.length?leaders.map((x,i)=>{const value=sortBy==='revenue'?x.revenue:x.qty,pct=Math.max(6,Math.round(value/max*100));return `<article class="sj-v26-rank-row"><b>${i+1}</b><div><strong>${esc(x.name)}</strong><small>${x.qty.toLocaleString('id-ID')} item · ${rupiah(x.revenue)} · ${x.transactions} transaksi</small><span><i style="width:${pct}%"></i></span></div><em>${sortBy==='revenue'?rupiah(x.revenue):`${x.qty.toLocaleString('id-ID')} item`}</em></article>`}).join(''):'<div class="sj-v26-empty">Belum ada produk terjual pada periode/filter ini.</div>'}</section><section class="sj-v26-history-list"><div class="sj-v26-section-head"><div><h2>Transaksi</h2><p>Klik transaksi untuk melihat produk yang dijual.</p></div></div>${visible.length?visible.map(tx=>{const lines=transactionItemLines(tx),units=lines.reduce((s,x)=>s+x.netQty,0);return `<button type="button" class="sj-v26-tx-row" data-sales-tx="${esc(txId(tx))}"><span class="ico">${renderIcon('receipt',{size:20})}</span><span><b>${esc(txId(tx))}</b><small>${esc(txTime(tx))} · ${esc(txShift(tx))} · ${esc(txCashier(tx))}</small><small>${units} item · ${esc(txMethod(tx))} · ${esc(txStatus(tx))}</small></span><strong>${rupiah(txTotal(tx))}</strong><span class="arrow">›</span></button>`}).join(''):'<div class="sj-v26-empty">Tidak ada transaksi pada filter ini.</div>'}</section></main>`;
}
...
export function installSalesHistoryRefinement(runtime=globalThis){
  ...
  const report=runtime?.SJReportFoundationV010,document=runtime?.document;if(!report?.state||!report?.Core)return Object.freeze({installed:false});
  const state={sortBy:'qty',filters:{},view:'default',transactionId:''};
  function root(){return document?.getElementById?.('lap-menu-view')||null}
  function model(){return report.state.model||{period:{label:'Periode terpilih'},transactions:[]}}
  ...
  function bindHistory(){const el=root();if(!el)return;el.querySelectorAll?.('[data-sales-filter]')?.forEach?.(node=>{const event=node.tagName==='INPUT'?'input':'change';node.addEventListener?.(event,()=>{state.filters[node.dataset.salesFilter]=node.value;openHistory()})});el.querySelectorAll?.('[data-sales-rank]')?.forEach?.(b=>b.addEventListener?.('click',()=>{state.sortBy=b.dataset.salesRank==='revenue'?'revenue':'qty';openHistory()}));el.querySelectorAll?.('[data-sales-tx]')?.forEach?.(b=>b.addEventListener?.('click',()=>openTransaction(b.dataset.salesTx)));el.querySelector?.('[data-sales-action="back"]')?.addEventListener?.('click',()=>{state.view='default';report.open?.()});el.querySelectorAll?.('[data-sales-period]')?.forEach?.(b=>b.addEventListener?.('click',async()=>{report.selectPeriod?.(b.dataset.salesPeriod);await report.refresh?.();openHistory()}));el.querySelector?.('[data-sales-period-date]')?.addEventListener?.('change',async e=>{if(!e.target?.value)return;report.selectPeriod?.({preset:'custom',explicit:{from:e.target.value,to:e.target.value}});await report.refresh?.();openHistory()})}
  function openHistory(){state.view='history';const m=model();show(renderSalesHistory({transactions:m.transactions||[],periodLabel:m.period?.label||m.summary?.period?.label||'Periode terpilih',sortBy:state.sortBy,filters:state.filters}));bindHistory();return true}
  function openTransaction(id){const tx=(model().transactions||[]).find(x=>txId(x)===String(id));if(!tx)return false;state.view='detail';state.transactionId=String(id);show(renderSalesTransactionDetail(tx,{core:report.Core,role:roleOf(runtime)}));root()?.querySelector?.('[data-sales-action="history"]')?.addEventListener?.('click',openHistory);return true}
  function enhance(){const el=root();if(!el||state.view!=='default')return false;if(el.querySelector?.('[data-sj-sales-history-open]'))return false;const host=el.querySelector?.('.sjr06-categories,.sj-rep0-detail-grid,.sj-rep0-launchers,.sj-rep0[data-view="cashier-shift"]')||el;const button=document?.createElement?.('button');if(!button)return false;button.type='button';button.dataset.sjSalesHistoryOpen='true';button.className='sj-v26-history-entry';button.innerHTML=`<span>${renderIcon('receipt',{size:21})}</span><span><b>Riwayat Penjualan</b><small>Lihat transaksi dan produk yang dijual · read-only</small></span><span>›</span>`;button.addEventListener?.('click',openHistory);host.appendChild?.(button);return true}
  const api=Object.freeze({installed:true,openHistory,openTransaction,enhance,state});try{Object.defineProperty(runtime,'__SJ_V26_SALES_HISTORY',{value:api,writable:false,configurable:false})}catch(_){}
  return api;
}
