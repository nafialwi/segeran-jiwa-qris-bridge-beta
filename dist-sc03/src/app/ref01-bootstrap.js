...
import { SETTINGS_GROUPS, SCREEN_FAMILIES, IMPLICIT_CAPABILITIES } from '../ui/refinement-contract.js';
...
const DESC=Object.freeze({
  products:'Harga, foto, urutan & ketersediaan',categories:'Kelola kelompok produk',materials:'Bahan baku, gudang & mutasi',customers:'Master pelanggan & referensi hutang',employees:'Master karyawan & kasbon',
  account:'Profil pengguna dan keamanan akun',users:'Akun Kasir dan Owner',devices:'Perangkat yang diizinkan dan statusnya',appearance:'Kepadatan dan tampilan pada perangkat ini',store:'Nama, logo dan QRIS toko',printer:'Thermal printer dan metode cetak',
  notifications:'Pusat notifikasi dan deep-link',security:'Status session, Firebase Auth dan sinkronisasi',activity:'Audit aktivitas sistem',diagnostics:'Firebase, printer dan error log',backup:'Cadangkan atau pulihkan data terverifikasi','delete-selected':'Aksi koreksi administratif yang destruktif'
});
...
function renderProfileHead(runtime,media){
  const user=getAuth(runtime)?.currentUser;const name=String(user?.displayName||'Owner Utama');const role='Owner / Pemilik';const photo=media.currentPhoto();const initials=(name.trim().split(/\s+/).map(x=>x[0]).join('').slice(0,2)||'OU').toUpperCase();
  return `<div class="sj-ref-settings-head"><div class="sj-ref-avatar">${photo?`<img src="${esc(photo)}" alt="Foto profil">`:esc(initials)}<span class="sj-ref-avatar-action">${renderIcon('camera',{size:13})}</span><input id="sj-ref-profile-file" type="file" accept="image/*" hidden></div><div class="sj-ref-profile-copy"><h2>${esc(name)}</h2><p>${esc(role)} · Akun aman & tersinkronisasi</p><div class="sj-ref-profile-actions"><button type="button" class="sj-ref-mini" data-ref01-profile="choose">Ganti foto</button>${photo?'<button type="button" class="sj-ref-mini" data-ref01-profile="remove">Hapus foto</button>':''}<button type="button" class="sj-ref-mini" data-ref01-feature="settings.account">Akun Saya</button></div></div></div>`;
}
function groupHtml(name,items){
  const sensitive=name==='Zona Sensitif';return `<section class="sj-ref-group${sensitive?' sj-ref-group-sensitive':''}"><h3>${esc(name)}</h3><div class="sj-ref-group-grid">${items.map(x=>`<button type="button" class="sj-ref-setting-card" data-ref01-feature="${esc(x.feature)}"><span class="sj-ref-card-icon">${renderIcon(x.icon,{size:18})}</span><span><b>${esc(x.label)}</b><small>${esc(DESC[x.key]||x.note||'')}</small></span>${renderIcon('chevron',{size:15})}</button>`).join('')}</div></section>`;
}
function renderSettingsLanding(document,runtime,sc03,media,openFeature){
  const root=document?.getElementById?.('mst-menu-view');if(!root||currentRole(sc03)!=='owner') return false;
  if(root.dataset?.ref01Settings==='true') return false;
  root.dataset.ref01Settings='true';root.innerHTML=`<div class="sj-ref-settings">${renderProfileHead(runtime,media)}${Object.entries(SETTINGS_GROUPS).map(([name,items])=>groupHtml(name,items)).join('')}</div>`;
  root.addEventListener?.('click',event=>{const p=event.target?.closest?.('[data-ref01-profile]');if(p){const action=p.dataset.ref01Profile;if(action==='choose')document.getElementById('sj-ref-profile-file')?.click?.();if(action==='remove')media.removeProfilePhoto().then(()=>{root.dataset.ref01Settings='';renderSettingsLanding(document,runtime,sc03,media,openFeature);notify(runtime,'Foto profil dihapus.','success')}).catch(e=>notify(runtime,e.message||'Gagal menghapus foto profil.','error'));return}const button=event.target?.closest?.('[data-ref01-feature]');if(button)openFeature(button.dataset.ref01Feature)});
  const input=document.getElementById?.('sj-ref-profile-file');input?.addEventListener?.('change',async event=>{const file=event.target.files?.[0];if(!file)return;try{await media.saveProfilePhoto(file);root.dataset.ref01Settings='';renderSettingsLanding(document,runtime,sc03,media,openFeature);notify(runtime,'Foto profil diperbarui.','success')}catch(e){notify(runtime,e.message||'Gagal memperbarui foto profil.','error')}});
  return true;
}
...
function enhanceImageRemove(document){
  const targets=[['edit-m-i','edit-img-preview'],['sjmux-q-img','sjmux-q-preview'],['set-logo-val','set-logo-preview'],['set-qris-val','set-qris-preview']];let count=0;
...
export function installRef01Runtime(runtime=globalThis,{sc03=runtime?.__SJ_SC03_RUNTIME,sc04=runtime?.__SJ_SC04_RUNTIME,observe=false}={}){
  ...
  const document=runtime?.document??null;installStyle(document);
  const media=createMediaLifecycle({imageAuthority:getImageAuthority(runtime),auth:getAuth(runtime)});const shift=createStaleShiftAdapter(runtime);
  ...
  const api=Object.freeze({phase:'REF-01',owner:OWNER,sc03,sc04,media,shift,backupActions,openFeature,enhance,stop:()=>observer?.disconnect?.(),snapshot:()=>Object.freeze({phase:'REF-01',owner:OWNER,familyCount:SCREEN_FAMILIES.length,families:SCREEN_FAMILIES,implicitCapabilities:IMPLICIT_CAPABILITIES,route:currentRoute(sc03)})});
