...
export function installSalesHistoryRefinement(runtime=globalThis){
  ...
  if(document?.addEventListener&&!document.__sjV27SalesHistoryDelegated){document.__sjV27SalesHistoryDelegated=true;document.addEventListener('click',event=>{const trigger=event.target?.closest?.('[data-sj-sales-history-open]');if(!trigger)return;event.preventDefault?.();openHistory()})}
