...
export function renderInventoryProcessV32({action='transfer',itemType='ingredient',row,location='warehouse',initialValues={}}={}){
  ...
  else fields=`<label>Lokasi<select data-v32-process-field="location"><option value="warehouse" ${location==='warehouse'?'selected':''}>Gudang</option><option value="outlet" ${location==='outlet'?'selected':''}>Gerai</option></select></label><label>Jumlah fisik aktual<input type="number" min="0" step="0.01" inputmode="decimal" data-v32-process-field="actual" value="${esc(initialValues?.actual??'')}" placeholder="0 ${esc(c.unit)}"></label><label>Alasan / catatan<input type="text" data-v32-process-field="note" value="${esc(initialValues?.note??'')}" placeholder="Contoh: selisih hitung / rusak"></label><aside class="sj-v32-process-preview"><b>Cek Stok Fisik</b><span>Gudang ${esc(qty(c.warehouse,c.unit))} · Gerai ${esc(qty(c.outlet,c.unit))}</span></aside>`;
...
export function renderInventoryWorkspaceV32({tab='summary',rows=[],cupRows=[],readOnly=false,recentMovements=[],recentActivities=[],query='',filter='ALL',intent='',selectedItemId='',mode='',productRows=[],process=null,manager=false,editorRow=undefined,actionQuery='',actionType='ALL',cupSetupValues={},reconciliationModel={summary:{total:0,unresolved:0,needsOpname:0,resolved:0},groups:[]},reconciliationExpandedDates=[],reconciliationSelectedRef='',reconciliationDate=''}={}){
  ...
  if(mode==='process')return `<div class="sj-v32-inv-shell" data-v32-inventory-shell data-active-tab="process"><div class="sj-v32-inv-body">${renderInventoryProcessV32({action:process?.action,itemType:process?.itemType,row:process?.row,location:process?.location,initialValues:{actual:process?.actual,note:process?.note}})}</div></div>`;
...
export function installInventoryWorkspaceV32(runtime=globalThis,{readShifts=readDateShifts}={}){
  ...
  async function openAction(action,itemType='ingredient',id='',options={}){
    ...
    const row=itemRow(itemType,id);if(!row)return false;state={...state,mode:'process',process:{action,itemType,row,location:options.location||'warehouse',actual:options.actual,note:options.note||'',origin:options.origin||'',reconciliationRef:options.reconciliationRef||''},detailId:'',actionQuery:'',actionType:'ALL'};render(host);return true;
  }
  ...
  async function handleClick(event,host){
    ...
    if(event.target?.closest?.('[data-v32-process-back],[data-v32-process-cancel]')){const p=state.process;if(p?.origin==='reconciliation'){state={...state,tab:'reconciliation',mode:'',process:null,detailId:'',reconSelectedRef:p.reconciliationRef||''};render(host);return}state.mode='';state.process=null;if(p?.itemType==='ingredient'){state.detailId=p.row?.id||''}else{host.style.display='none';runtime?.__SJ_V26_FINISHED_WAREHOUSE?.openProductDetail?.(p?.row?.id||'');return}render(host);return}
    ...
    const reconOpname=event.target?.closest?.('[data-r10-opname-ref]');if(reconOpname){const ref=String(reconOpname.dataset.r10OpnameRef||''),item=reconciliationModel.groups?.flatMap?.(group=>group.items||[])?.find?.(row=>String(row.reference)===ref)||null;try{const draft=buildCupReconOpnameDraft(item||{});await openAction('opname','ingredient',draft.ingredientId,draft)}catch(e){runtime?.alert?.(e?.message||'Opname rekonsiliasi belum dapat dibuka.')}return}
