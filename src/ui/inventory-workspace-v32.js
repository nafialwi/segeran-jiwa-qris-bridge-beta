...
export function routeLegacyInventoryTabV32(tab='summary'){
  ...
  if(value==='ingredients')return{kind:'stock-items'};
...
function moreHTML(){
  return `<section class="sj-v32-inv-more"><button type="button" data-v32-inventory-action="recipes"><span>${renderIcon('product',{size:21})}</span><b>Resep Produk &amp; Ukuran</b><small>Atur komposisi bahan untuk produk yang memang menggunakan resep.</small></button><button type="button" data-v32-inventory-action="movements"><span>${renderIcon('history',{size:21})}</span><b>Riwayat Stok</b><small>Lihat aktivitas stok terbaru dengan bahasa operasional.</small></button><button type="button" data-v32-inventory-choose="purchase"><span>${renderIcon('customers',{size:21})}</span><b>Supplier &amp; Pembelian</b><small>Pilih item lebih dulu, lalu catat barang masuk.</small></button><button type="button" data-v32-inventory-action="cost"><span>${renderIcon('cash',{size:21})}</span><b>HPP &amp; Biaya Produk</b><small>Lihat biaya bahan dan biaya produk yang tersedia.</small></button><button type="button" data-v32-inventory-manage="ingredients"><span>${renderIcon('settings',{size:21})}</span><b>Kelola Item Stok</b><small>Tambah bahan dan atur batas stok Gerai maupun Gudang.</small></button></section>`;
}
...
export function installInventoryWorkspaceV32(runtime=globalThis){
  ...
  async function load(){
    const [raw,outlet,recentMovements]=await Promise.all([
      repository.readWorkspaceState(),
      repository.readLegacyStock(),
      repository.readRecentMovements({limit:120})
    ]);
    const inv=raw||{},activitySnapshot={...inv,movements:recentMovements||{}};
    applyCupRows(inv);
    ...
    activities=inventoryActivityTimeline(activitySnapshot,120).filter(a=>!legacyCupIds.has(String(a.itemId)));
  ...
  async function openStockItems(){
    ...
    if(!openLoadTask)openLoadTask=load().finally(()=>{openLoadTask=null});
    try{await openLoadTask;if(requestSeq===openRequestSeq)render(host)}
    catch(_){if(requestSeq===openRequestSeq&&content)content.innerHTML='<div class="sj-v32-inv-empty">Item Stok belum dapat dimuat. Tidak ada data yang diubah.</div>'}
    return true;
  }
  function wrappedOpen(tab){
    ...
    if(route.kind==='stock-items')return openStockItems();
