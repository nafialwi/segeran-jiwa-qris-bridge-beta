...
function chartHTML(transactions=[],state={}){
  const chartOptions={scope:state.scope,shift:state.shift,week:state.week,from:state.customFrom,to:state.customTo},buckets=chartBucketsForScope(transactions,chartOptions),bucketMode=chartBucketModeForScope(chartOptions),metric=state.metric||'revenue';
  const key=metric==='transactions'?'transactions':metric==='items'?'items':'revenue',max=Math.max(1,...buckets.map(x=>num(x[key]))),label=metric==='transactions'?'Transaksi':metric==='items'?'Item':'Omzet',bucketLabel={hour:'jam',shift:'shift',day:'hari',week:'minggu'}[bucketMode]||'periode';
  return `<section class="sjv28-card sjv28-chart"><div class="sjv28-section-head"><div><h2>Grafik ${label}</h2><p>Otomatis per ${bucketLabel} sesuai scope aktif.</p></div><div class="sjv28-toggle"><button data-v29-metric="revenue" data-v28-metric="revenue" class="${metric==='revenue'?'active':''}">Omzet</button><button data-v29-metric="transactions" data-v28-metric="transactions" class="${metric==='transactions'?'active':''}">Transaksi</button><button data-v29-metric="items" data-v28-metric="items" class="${metric==='items'?'active':''}">Item</button></div></div>${buckets.length?`<div class="sjv28-bars" data-v32-chart-bucket="${bucketMode}">${buckets.map(x=>`<div class="sjv28-bar-col" title="${esc(x.label)} · ${key==='revenue'?rupiah(x[key]):count(x[key])}"><div class="sjv28-bar-track"><i style="height:${Math.max(4,Math.round(num(x[key])/max*100))}%"></i></div><small>${esc(x.label)}</small></div>`).join('')}</div>`:'<div class="sjv28-empty">Belum ada data untuk grafik.</div>'}</section>`;
}
...
function soldProductsHTML(analytics,compat){
  const rows=analytics.topByQty||[];
  return `<section class="sjv28-card sjv29-sold"><div class="sjv28-section-head"><div><h2>Barang Terjual</h2><p>Jumlah/omzet scope aktif · stok adalah saldo Gerai saat ini, bukan snapshot historis.</p></div></div><div class="sjv29-sold-list sjv30-sold-list">${rows.length?rows.map(row=>{let stock=null;try{stock=compat?.outletStock?compat.outletStock(row.id):null}catch(_){}const stockText=stock===null||stock===undefined?'Belum tersedia':count(stock);return `<div class="sjv30-sold-row"><span class="sjv30-sold-product"><b>${esc(row.name)}</b><small>${count(row.transactions)} transaksi</small></span><span class="sjv30-sold-metrics"><span><small>Terjual</small><strong>${count(row.qty)}</strong></span><span><small>Omzet</small><strong>${rupiah(row.revenue)}</strong></span><span><small>Stok Gerai Saat Ini</small><strong>${stockText}</strong></span></span></div>`}).join(''):'<div class="sjv28-empty">Belum ada barang terjual pada scope ini.</div>'}</div></section>`;
}
function historyPreviewHTML(transactions=[]){
  ...
  return `<section class="sjv28-card"><div class="sjv28-section-head"><div><h2>Riwayat Penjualan</h2><p>Scope aktif · klik transaksi untuk detail item.</p></div></div><div class="sjv28-history-preview">${rows.length?rows.map(tx=>{const id=String(tx.id??tx._key??tx.transactionId??'');return `<button type="button" data-v29-history-tx="${esc(id)}"><span><b>${esc(id||'Transaksi')}</b><small>${esc(String(tx.paymentMethod??tx.method??tx.payment?.method??'-'))}</small></span><strong>${rupiah(tx?.pricing?.total??tx?.grandTotal??tx?.total)}</strong></button>`}).join(''):'<div class="sjv28-empty">Belum ada transaksi.</div>'}</div><button type="button" class="sj-v26-history-entry sjv30-report-link" data-sj-sales-history-open="true"><span>${renderIcon('receipt',{size:20})}</span><span><b>Lihat semua produk terjual</b><small>Cari ID, filter pembayaran/kasir, dan detail item · read-only</small></span><span>›</span></button></section>`;
}
