...
function options(selected='',catalog=CUP_CATALOG_V34){
  const mapped=(Array.isArray(catalog)&&catalog.length?catalog:CUP_CATALOG_V34).filter(x=>x.categoryDefault!==false);
...
export function cupSaleConsumptionV34(cart=[],catalog=CUP_CATALOG_V34){
  ...
  for(const line of cart||[]){
    ...
    if(!isCupCodeV34(code,specs)||qty<=0)continue;
    out[code]+=qty;
  }
  return Object.freeze(out);
}

export function renderCategoryCupMappingV34(categoryRows=[],menu=[],{readOnly=false,catalog=CUP_CATALOG_V34}={}){
  const rows=(categoryRows||[]).map(category=>{const selected=categoryCode(menu,category);return `<article class="sj-v34-cat-cup-row"><div><b>${esc(category)}</b><small>Terapkan ke produk aktif; produk tetap bisa dioverride lewat Edit Produk.</small></div><select data-v34-cup-category="${esc(category)}" ${readOnly?'disabled aria-disabled="true"':''}>${options(selected,catalog)}</select><button type="button" data-v34-cup-category-apply="${esc(category)}" ${readOnly?'disabled aria-disabled="true"':''}>${readOnly?'🔒 READ ONLY':'Terapkan Cup'}</button></article>`}).join('');
...
function syncCpSelect(select,catalog=CUP_CATALOG_V34){
  if(!select)return false;const selected=String(select.value||'');const specs=Array.isArray(catalog)&&catalog.length?catalog:CUP_CATALOG_V34;
  const legacy=cupSpecByCodeV34(selected,CUP_CATALOG_V34),visible=specs.some(x=>x.code===selected)||!legacy?specs:[...specs,legacy];
  select.innerHTML=options(selected,visible);select.value=selected;return true;
}

export function installCupProductCostingV34(runtime=globalThis,{repository=createInventoryRepository({db:runtime?.firebase?.database?.()}),autoEnhance=true,catalogService=runtime?.__SJ_CUP_CATALOG_V1}={}){
  ...
  async function refresh(){
    try{await catalogService?.refresh?.()}catch(_){}
    try{
  ...
  const saleUsage=cart=>cupSaleConsumptionV34(cart,fullCatalog());
  const costForCode=code=>{
    const key=text(code).toLowerCase(),spec=cupSpecByCodeV34(key,fullCatalog()),row=cachedCupRows.find(x=>x?.code===key);
  ...
  function enhance(){
    const catalog=activeCatalog();syncCpSelect(document?.getElementById?.('new-cp'),catalog);syncCpSelect(document?.getElementById?.('edit-m-cp'),catalog);
    const root=document?.getElementById?.('mst2');if(!root)return false;let panel=root.querySelector?.('[data-v34-category-cup]');const html=renderCategoryCupMappingV34(categories(runtime),menuRows(runtime),{readOnly,catalog});
  ...
  try{document?.addEventListener?.('sj:cup-catalog-changed',()=>{try{enhance()}catch(_){}})}catch(_){}
