import { CUP_CATALOG_V34, buildCupInventoryRowsV34, buildCupLocalSimulationRowsV34, buildCupOutletOpnameDraftsV34, cupInboundFromMovementsV34, ensureCupLocalSimulationStoreV34, reconcileCupClosingAuthorityV34, reconcileCupShiftV34, theoreticalCupUsageV34 } from '../domain/packaging-cup-v34.js';
...
export function renderCupClosingPanelV34(cupRows=[],{reconciliation={rows:[]},readOnly=false,closingValues=null,openingKnown=true,opnameDrafts=[],simulationInbound={}}={}){
  ...
  return `<section class="sj-v34-cup-shift-panel" data-v34-cup-closing-panel><header><div><small>Kontrol Kemasan · rekonsiliasi fisik</small><h4>Hitung Cup Akhir</h4><p>Expected Closing = awal + masuk − pemakaian transaksi; bandingkan dengan Physical Closing. Variance = expected − physical.</p></div>${readOnly?'<em>LOCAL QA · READ ONLY</em>':''}</header>${simulationNote}${legacyNote}<div class="sj-v34-cup-close-grid">${fields}</div>${syncNote}</section>`;
}
...
export function installCupShiftControlV34(runtime=globalThis,{inventoryWorkspace=runtime?.__SJ_V32_INVENTORY_WORKSPACE}={}){
  ...
  async function computeClose(closingValues=null){
    ...
    const reconciliation=reconcileCupClosingAuthorityV34({openingKnown,opening,inbound,systemClosing,closing,theoretical}),opnameDrafts=buildCupOutletOpnameDraftsV34(currentCups,closing);
