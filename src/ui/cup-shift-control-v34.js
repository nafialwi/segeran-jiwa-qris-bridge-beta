import { CUP_CATALOG_V34, buildCupInventoryRowsV34, buildCupLocalSimulationRowsV34, buildCupOutletOpnameDraftsV34, cupInboundFromMovementsV34, cupStockComponentUsageV34, ensureCupLocalSimulationStoreV34, reconcileCupClosingAuthorityV34, reconcileCupShiftV34 } from '../domain/packaging-cup-v34.js';
...
export function renderCupClosingPanelV34(cupRows=[],{reconciliation={rows:[]},readOnly=false,closingValues=null,openingKnown=true,opnameDrafts=[],simulationInbound={}}={}){
  ...
  const fallback=reconciliation?.authority==='INVENTORY_FALLBACK';
  const controlsLocked=false;
  const legacyNote=fallback?'<div class="sj-v34-cup-note warn"><b>Mode kompatibilitas shift lama.</b> Shift ini tidak memiliki hitung cup awal. Sistem menampilkan saldo Inventory V2 saat ini sebagai pembanding agar shift tetap dapat ditutup. Hitung fisik akhir tetap wajib; hasil ini menjadi continuity untuk shift berikutnya.</div>':!openingKnown?'<div class="sj-v34-cup-note warn">Data hitung cup awal belum tersedia. Gunakan saldo sistem yang ditampilkan dan pastikan hitung fisik akhir lengkap.</div>':'';
  ...
  const fields=CUP_CATALOG_V34.map(spec=>{const row=byCode.get(spec.code)||{code:spec.code,name:spec.name,opening:0,inbound:0,closing:'',physicalUsed:0,theoreticalUsed:0,variance:0};const simInbound=readOnly&&openingKnown?`<label class="sj-v34-cup-sim-inbound"><span>Cup Masuk simulasi</span><input type="number" min="0" step="1" inputmode="numeric" data-v34-cup-inbound-sim="${esc(spec.code)}" value="${esc(num(simulationInbound?.[spec.code]))}"></label>`:'';const rawClosing=closingValues===null?row.closing:(closingValues?.[spec.code]??'');const hasClosing=rawClosing!==null&&rawClosing!==undefined&&String(rawClosing).trim()!=='';const variance=hasClosing?num(row.variance):0,disabled=controlsLocked?' disabled aria-disabled="true"':'';const reason=hasClosing&&variance!==0?`<label class="sj-v34-cup-reason"><span>Alasan selisih</span><select data-v34-cup-reason="${esc(spec.code)}" required${disabled}><option value="">Pilih alasan...</option>${REASONS.map(([id,label])=>`<option value="${id}">${esc(label)}</option>`).join('')}</select><input data-v34-cup-reason-note="${esc(spec.code)}" placeholder="Catatan opsional"${disabled}></label>`:'';const expectedValue=row.expectedClosing===undefined?num(row.opening)+num(row.inbound)-num(row.theoreticalUsed):num(row.expectedClosing),expectedClosing=(openingKnown||fallback)?`${esc(expectedValue)} pcs`:'—',physicalClosing=hasClosing?`${esc(num(row.physicalClosing??rawClosing))} pcs`:'—',varianceText=hasClosing&&(openingKnown||fallback)?`${esc(variance)} pcs`:'—',legacyRecon=hasClosing?`${fallback?'System Inventory':'Fisik terpakai'} ${fallback?expectedValue:num(row.physicalUsed)} pcs; Transaksi ${num(row.theoreticalUsed)} pcs; Selisih ${variance} pcs`:`${fallback?'System Inventory':'Fisik terpakai'} ${fallback?expectedValue:'—'}; Transaksi ${num(row.theoreticalUsed)} pcs; Selisih —`;return `<article class="sj-v34-cup-close-row" data-v34-cup-close-row="${esc(spec.code)}"><header><b>${esc(spec.name)}</b><span>${fallback?'System Inventory '+esc(expectedValue)+' pcs':`Awal ${openingKnown?esc(num(row.opening)):'—'} · Masuk ${openingKnown?esc(num(row.inbound)):'—'}`}</span></header>${simInbound}<label><span>Hitung fisik akhir</span><input type="number" min="0" step="1" inputmode="numeric" required data-v34-cup-closing="${esc(spec.code)}" value="${hasClosing?esc(num(rawClosing)):''}"${disabled}></label><div class="sj-v34-cup-recon" aria-label="${esc(legacyRecon)}"><span><small>${fallback?'System Closing':'Expected Closing'}</small><b>${expectedClosing}</b></span><span><small>Physical Closing</small><b>${physicalClosing}</b></span><span class="${hasClosing&&(openingKnown||fallback)&&variance!==0?'warn':'ok'}"><small>Variance</small><b>${varianceText}</b></span></div>${reason}</article>`}).join('');
  ...
  return `<section class="sj-v34-cup-shift-panel" data-v34-cup-closing-panel><header><div><small>Kontrol Kemasan · rekonsiliasi fisik</small><h4>Hitung Cup Akhir</h4><p>Expected Closing = awal + masuk − pemakaian komponen neto; bandingkan dengan Physical Closing. Variance = physical − expected.</p></div>${readOnly?'<em>LOCAL QA · READ ONLY</em>':''}</header>${simulationNote}${legacyNote}<div class="sj-v34-cup-close-grid">${fields}</div>${syncNote}</section>`;
}
...
export function installCupShiftControlV34(runtime=globalThis,{inventoryWorkspace=runtime?.__SJ_V32_INVENTORY_WORKSPACE}={}){
  ...
  let cupRows=[],dayRows={},pendingStart=null,pendingClose=null,closeContext=null,closeEnhanceTask=null,openingRefreshTask=null;
  ...
  async function computeClose(closingValues=null){
    ...
    const {raw,cupRows:currentCups}=await refreshCupRows(),componentAuthority=cupStockComponentUsageV34(raw,currentCups,shiftKey,{startTs,endTs}),theoretical=componentAuthority.netUsage,actualInbound=cupInboundFromMovementsV34(raw,currentCups,shiftKey,{startTs,endTs}),inbound=readOnly?Object.fromEntries(CUP_CATALOG_V34.map(x=>[x.code,num(actualInbound?.[x.code])+num(localSimulation.inboundCounts?.[x.code])])):actualInbound,closing=closingValues||Object.fromEntries(CUP_CATALOG_V34.map(x=>[x.code,''])),systemClosing=Object.fromEntries(CUP_CATALOG_V34.map(x=>[x.code,num(currentCups.find(row=>row?.code===x.code)?.outletQty)]));
    const reconciliation=reconcileCupClosingAuthorityV34({openingKnown,opening,inbound,systemClosing,closing,theoretical,usageAuthority:componentAuthority.authority}),opnameDrafts=buildCupOutletOpnameDraftsV34(currentCups,closing);
    return {shiftKey,sid,opening,openingKnown,inbound,theoretical,componentAuthority,closing,reconciliation,opnameDrafts};
  }
  ...
  async function ensureClosingPanel(){
    const modal=document.querySelector?.('#modal-sjshift-close .modal');if(modal?.querySelector?.('[data-v34-cup-closing-panel]'))return true;
    if(closeEnhanceTask)return closeEnhanceTask;
    closeEnhanceTask=Promise.resolve().then(enhanceClosing).finally(()=>{closeEnhanceTask=null});return closeEnhanceTask;
  }
  ...
  const api=Object.freeze({installed:true,refresh:refreshCupRows,enhanceOpening,enhanceClosing:ensureClosingPanel,cupRows:()=>cupRows.slice(),readOnly});
