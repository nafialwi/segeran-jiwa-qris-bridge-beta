...
const OUT=join(ROOT,'dist-sc03');
const ENTRY='<script type="module" src="./src/sc03-entry.js" data-sj-sc03-entry="true"></script>';

rmSync(OUT,{recursive:true,force:true});
mkdirSync(OUT,{recursive:true});
cpSync(join(ROOT,'src'),join(OUT,'src'),{recursive:true});
const legacy=readFileSync(BASE,'utf8');
if((legacy.match(/<\/body>/gi)||[]).length!==1) throw new Error('SC03_BUILD_BODY_ANCHOR_INVALID');
...
console.log(`SC-03 candidate build: dist-sc03/index.html ${sha}`);
