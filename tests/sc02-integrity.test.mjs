...
test('SC-02 extracted data/domain source has zero direct Firebase mutation calls',()=>{
  const offenders=[];const re=/\.(?:set|update|transaction|remove)\s*\(/;
  for(const file of [...walk(join(ROOT,'src','data')),...walk(join(ROOT,'src','domain'))]){const text=readFileSync(file,'utf8');if(re.test(text))offenders.push(file)}
