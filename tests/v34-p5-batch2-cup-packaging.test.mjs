...
test('CUP-CONTROL-V1 defines six Cup masters while Paper 10 Oz stays outside sale mapping',()=>{
  assert.deepEqual(CUP_CATALOG_V34.map(x=>x.code),['c10','c10p','c16','c22p','c22d','c22o']);
  assert.equal(cupSpecByCodeV34('c10p').name,'Cup Paper 10 Oz');assert.equal(cupSpecByCodeV34('c10p').saleMapping,false);assert.ok(CUP_CATALOG_V34.every(x=>x.unit==='pcs'));
});
