...
function stripPerItemDelete(html=''){
  return String(html).replace(/<button\b[^>]*\bdata-remove\b[^>]*>[\s\S]*?<\/button>/gi,'');
}
...
export function installProductionSalesStability(runtime=globalThis){
  ...
  if(final&&typeof final.productCard==='function'){
    ...
    final.productCard=function(product){
      const raw=baseCard(product);
      const html=String(raw).replace(/<small\b[^>]*class="[^"]*sjvc01-context[^"]*"[^>]*>\s*Siap dijual\s*<\/small>/i,'');
      const qty=compat.productQty?.(product?.id)||0;if(qty<=0)return html;
  ...
  const cartOwner=runtime?.SJFinalRefinementVC01A2;
  if(cartOwner&&typeof cartOwner.cartRow==='function'&&!cartOwner.__sjV29NoLineDelete){
    const baseCartRow=cartOwner.cartRow.bind(cartOwner);
    cartOwner.cartRow=function(...args){return stripPerItemDelete(baseCartRow(...args))};
    try{Object.defineProperty(cartOwner,'__sjV29NoLineDelete',{value:true,configurable:false})}catch(_){cartOwner.__sjV29NoLineDelete=true}
  }
...
const GROUPED_HEADER_SELECTOR='.sjvc02-head,.sjr01-settings-header';
function manualSyncTarget(host,document){
  if(!host?.matches?.(GROUPED_HEADER_SELECTOR))return host;
  let group=host.querySelector?.('.sj-v29-header-actions');
  if(group)return group;
  group=document?.createElement?.('div');if(!group)return host;
  group.className='sj-v29-header-actions';
  const help=Array.from(host.children||[]).find(node=>node?.matches?.('.sjvc02-help,.sjr01-help'));
